...
_batch_lock_obj: asyncio.Lock | None = None
...
def _get_batch_lock() -> asyncio.Lock:
    global _batch_lock_obj
    if _batch_lock_obj is None:
        _batch_lock_obj = asyncio.Lock()
    return _batch_lock_obj
...
def ensure_batch_job() -> None:
    ...
    # 检查任务是否已存在
    try:
        existing_job = scheduler.get_job(BATCH_JOB_ID)
